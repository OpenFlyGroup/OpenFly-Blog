...
const Header: React.FC = () => {
  ...
  return (
    <div className='navbar lg:px-16 bg-base-100'>
          ...
          >
            {NavLinks.map((item) => (
              <li key={item.id}>
                <Link
                  href={item.path}
                  className={`${isActive(item.path, pathname) ? 'active' : ' '} text-sm font-[500] leading-6 text-base-100 hover:text-primary duration-200`}
                >
                  {item.name}
                </Link>
              </li>
